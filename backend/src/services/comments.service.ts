...
export class CommentsService {
    ...
    return transaction(async (client) => {
      ...
      await client.query(
        `INSERT INTO activity_logs (project_id, user_id, entity_type, entity_id, action, changes)
         VALUES ($1, $2, $3, $4, $5, $6)`,
        [projectId, userId, 'comment', comment.id, 'created', JSON.stringify({ task_id: taskId })]
      );
  ...
  static async listComments(taskId: number, userId: number): Promise<CommentWithUser[]> {
  ...
  // Get comment by ID
  static async getCommentById(commentId: number, userId: number): Promise<CommentWithUser> {
    const result = await query(
      `SELECT
        c.*,
        u.first_name || ' ' || u.last_name as user_name,
       ...
       INNER JOIN project_members pm ON t.project_id = pm.project_id
       WHERE c.id = $1 AND pm.user_id = $2 AND c.deleted_at IS NULL AND t.deleted_at IS NULL`,
      [commentId, userId]
    );
  ...
  static async updateComment(
    ...
    return transaction(async (client) => {
      ...
      await client.query(
        `INSERT INTO activity_logs (project_id, user_id, entity_type, entity_id, action, changes)
         VALUES ($1, $2, $3, $4, $5, $6)`,
        [comment.project_id, userId, 'comment', commentId, 'updated', JSON.stringify({ content })]
      );
    ...
    return transaction(async (client) => {
      ...
      await client.query(
        `INSERT INTO activity_logs (project_id, user_id, entity_type, entity_id, action)
         VALUES ($1, $2, $3, $4, $5)`,
        [comment.project_id, userId, 'comment', commentId, 'deleted']
      );
    });
  }
  ...
  static async getProjectActivity(
    ...
    const countResult = await query(
      `SELECT COUNT(*) as total FROM (${queryText}) as filtered`,
      params
    );
